...
import { Layout, Briefcase, History, LogOut, Cpu } from 'lucide-react';

const EmployeeSidebar = ({ activeTab, setActiveTab, currentUser, handleSignOut }) => {
  const tabs = [
    { id: 'dashboard', label: 'Dashboard', icon: Layout },
    { id: 'work', label: 'Work Console', icon: Briefcase },
    { id: 'archive', label: 'Project Archive', icon: History },
  ];
  ...
  return (
    <aside className="w-80 h-screen glass-card !rounded-none !border-y-0 !border-l-0 flex flex-col relative z-20 shrink-0">
       <div className="p-10 flex items-center gap-4 mb-10">
          ...
          <div>
             <h2 className="text-xl font-bold text-white tracking-tighter uppercase">Nexus</h2>
             <p className="text-[9px] font-black text-brand-primary uppercase tracking-[0.3em]">Employee Terminal</p>
          </div>
       </div>
       ...
       <div className="p-8 border-t border-white/5 space-y-4">
          <div className="flex items-center gap-4 px-4 py-4 bg-white/5 border border-white/10 rounded-3xl">
             ...
             <div className="flex-1 min-w-0">
                ...
                <p className="text-[8px] font-black text-slate-500 uppercase truncate">Sector: Operations</p>
             </div>
          </div>
