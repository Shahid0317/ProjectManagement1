...
import { 
  Shield, Layout, Users, PlusCircle, Briefcase, Activity, LogOut, X, FileCheck, Award
} from 'lucide-react';
         ...
         <nav className="flex-1 px-6 space-y-3 shrink-0">
            {tabs.map((tab) => (
              <button
                ...
                className={`w-full flex items-center gap-4 px-6 py-4.5 rounded-[2rem] transition-all duration-500 group relative ${
                  activeTab === tab.id 
              ...
              >
                 ...
                 <span className="text-xs font-black uppercase tracking-wider leading-none whitespace-nowrap">{tab.label}</span>
                 {activeTab === tab.id && (
                   <div className="absolute right-5 w-1.5 h-1.5 bg-brand-primary rounded-full animate-pulse"></div>
                 )}
              </button>
            ))}
         </nav>
