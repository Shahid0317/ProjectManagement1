...
import { 
  Shield, Layout, Users, PlusCircle, Briefcase, Activity, LogOut 
} from 'lucide-react';

const AdminSidebar = ({ activeTab, setActiveTab, currentUser, handleSignOut }) => {
  return (
    <aside className="w-80 h-screen glass-card !rounded-none !border-y-0 !border-l-0 flex flex-col relative z-20 shrink-0">
       <div className="p-10 flex items-center gap-4 mb-10">
          ...
          <div>
             <h2 className="text-xl font-bold text-white tracking-tighter">Project Terminal</h2>
             <p className="text-[9px] font-black text-brand-primary uppercase tracking-[0.3em]">Administrative Node Alpha</p>
          </div>
       </div>
       ...
       <nav className="flex-1 px-6 space-y-3">
          {[
            ...
            { id: 'project_assignment', label: 'Project Control', icon: PlusCircle },
            { id: 'ledger', label: 'Project Ledger', icon: Briefcase },
            { id: 'attendance', label: 'Surveillance', icon: Activity },
          ].map((tab) => (
            <button
              key={tab.id}
       ...
       <div className="p-8 border-t border-white/5 space-y-4">
          <div className="flex items-center gap-4 px-4 py-4 bg-white/5 border border-white/10 rounded-3xl">
             ...
             <div className="flex-1 min-w-0">
                ...
                <p className="text-[8px] font-black text-slate-500 uppercase truncate">Sector Administrator</p>
             </div>
          </div>
